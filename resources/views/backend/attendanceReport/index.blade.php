...
@section('content')
    ...
    <section class="content">
        <div class="box box-primary">
            ...
            @if(empty($_GET))
                {!! Form::open([
                               'action' => 'Backend\AttendanceController@reportIndex',
                               'method' => 'GET',
                               'role' => 'form',
                           ]) !!}
            @else
                                    ...
                                    @foreach($students as $student)
                                        <tr>
                                            ...
                                            @for($day = 1; $day <= $days ;$day++)
                                                ...
                                                @elseif(date('Y-m-d') <  date('Y-m-d',strtotime($year.'-'.$month.'-'.$day)))
                                                    <td>-</td>
                                                @else
                                                    <td>
                                                        @if($student->attendances()->where('date', date('Y-m-d',strtotime($year.'-'.$month.'-'.$day)))->count() > 0)
                                            ...
                                            @endfor
                                            <td>{{$days - $student->attendances()->whereYear('date', date('Y',strtotime($year)))->whereMonth('date',$month)->count()}}</td>
                                            <td>{{$student->attendances()->whereYear('date', date('Y',strtotime($year)))->whereMonth('date',$month)->count()}}</td>
                                            <td>{{$days}}</td>
                                        </tr>
